...
void GitIndexEntry::Initialize(Handle<v8::Object> target) {
  ...
  target->Set(NanSymbol("IndexEntry"), _constructor_template);
}
...
NAN_METHOD(GitIndexEntry::New) {
  ...
  if (args.Length() == 0 || !args[0]->IsExternal()) {
    NanThrowError("git_index_entry is required.");
  }

  //GitIndexEntry* object = new GitIndexEntry((git_index_entry *) External::Unwrap(args[0]));
  GitIndexEntry* object = ObjectWrap::Unwrap<GitIndexEntry>(args[0]->ToObject());
