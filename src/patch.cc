...
void GitPatch::Initialize(Handle<v8::Object> target) {
  ...
  target->Set(NanSymbol("Patch"), _constructor_template);
}
...
NAN_METHOD(GitPatch::New) {
  ...
  if (args.Length() == 0 || !args[0]->IsExternal()) {
    NanThrowError("git_diff_patch is required.");
  }

  //GitPatch* object = new GitPatch((git_diff_patch *) External::Unwrap(args[0]));
  GitPatch* object = ObjectWrap::Unwrap<GitPatch>(args[0]->ToObject());
...
NAN_METHOD(GitPatch::Stats) {
  ...
  Handle<Value> to;
    ...
    toReturn->Set(NanSymbol("total_context"), to);

      to = NanNew<Integer>(total_additions);
    toReturn->Set(NanSymbol("total_additions"), to);

      to = NanNew<Integer>(total_deletions);
    toReturn->Set(NanSymbol("total_deletions"), to);
...
NAN_METHOD(GitPatch::Hunk) {
  NanScope();
    ...
    NanThrowError("Number hunk_idx is required.");
  }
  ...
  if (range != NULL) {
    ...
    toReturn->Set(NanSymbol("range"), to);

      to = NanNew<String>(header);
    toReturn->Set(NanSymbol("header"), to);

      to = NanNew<Uint32>(header_len);
    toReturn->Set(NanSymbol("headerLength"), to);

      to = NanNew<Uint32>(lines_in_hunk);
    toReturn->Set(NanSymbol("lines"), to);
...
NAN_METHOD(GitPatch::Line) {
  ...
  if (args.Length() == 1 || !args[1]->IsUint32()) {
    NanThrowError("Number line_of_hunk is required.");
  }
  ...
  Handle<Value> to;
    ...
    toReturn->Set(NanSymbol("lineOrigin"), to);

      to = NanNew<String>(content, content_len);
    toReturn->Set(NanSymbol("content"), to);

      to = NanNew<Uint32>(content_len);
    toReturn->Set(NanSymbol("length"), to);

      to = NanNew<Int32>(old_lineno);
    toReturn->Set(NanSymbol("oldLineNumber"), to);

      to = NanNew<Int32>(new_lineno);
    toReturn->Set(NanSymbol("newLineNumber"), to);
